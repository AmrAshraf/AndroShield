** Download/Clone the project and extract/put the folders under C:\

(1) Droidbot installation instructions:
- Perform the following CMD instructions:
	C:\Python27\Scripts\pip.exe install -e C:\droidbot-master
	pip uninstall androguard
	pip install androguard==3.0.1

(2) AndroShieldApp web application database installation
- Run SQL script on a dummy SQL database and change the paths in the file C:\AndroShieldDB.publish.sql
...
- To run the emulator :   C:\AndroidEmulator\emulator\emulator.exe -avd Nexus_4_API_26_NoSkin
- To kill the emulator (After Finishing) : adb emu kill

(4) Run AndroidShieldApp through visual studio and make sure C:\production\androShieldCPP.dll dependency exists
